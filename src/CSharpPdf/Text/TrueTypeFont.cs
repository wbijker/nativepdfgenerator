...
/// font (Subtype /TrueType + FontFile2). The sfnt tables are parsed to extract the
/// metrics and character-to-glyph mapping the PDF font dictionary needs; the glyph
/// outlines themselves are embedded verbatim. This implements single-byte
/// (WinAnsi/Latin-1) fonts; full Unicode would need a Type0/CIDFont.
/// </summary>
public sealed class TrueTypeFont : EmbeddedFont
{
    ...
    private readonly ushort[] _advances;   // advance width per glyph id (length numberOfHMetrics)
    private readonly int[] _gidByCode = new int[256];
    private readonly int[] _charWidths;    // codes First..Last, 1000-unit space
    private readonly string _psName;
    private readonly int _ascent, _descent, _capHeight, _xHeight; // 1000-unit space (descent negative)
    private readonly int _xMin, _yMin, _xMax, _yMax;             // 1000-unit space
    ...
    private TrueTypeFont(byte[] program)
    {
        ...
        _ = U16(program, maxp + 4); // numGlyphs (not needed directly)
        ...
        if (offsets.TryGetValue("OS/2", out int os2))
        {
            ...
            ascent = S16(program, os2 + 68);   // sTypoAscender
            descent = S16(program, os2 + 70);  // sTypoDescender
    ...
    public override FontVerticalMetrics GetVerticalMetrics(double fontSize)
    {
        ...
        return new FontVerticalMetrics(
            Ascent: _ascent * s,
            Descent: -_descent * s, // _descent is negative; expose a positive depth
            LineGap: 0,
            CapHeight: _capHeight * s,
            XHeight: _xHeight * s);
    }

    // ----- EmbeddedFont overrides -----
    ...
    protected override PdfDictionary BuildDescriptor() => new()
    {
        ["Type"] = new PdfName("FontDescriptor"),
        ["FontName"] = new PdfName(_psName),
        ["Flags"] = new PdfNumber(Flags()),
        ["FontBBox"] = new PdfArray(new PdfNumber(_xMin), new PdfNumber(_yMin), new PdfNumber(_xMax), new PdfNumber(_yMax)),
        ["ItalicAngle"] = new PdfNumber(_italicAngle),
        ["Ascent"] = new PdfNumber(_ascent),
        ["Descent"] = new PdfNumber(_descent),
        ["CapHeight"] = new PdfNumber(_capHeight),
        ["StemV"] = new PdfNumber(_weightClass >= 600 ? 140 : 80),
    };

    private int Flags()
    {
        int flags = 32; // Nonsymbolic (bit 6)
        if (_fixedPitch) flags |= 1;     // FixedPitch (bit 1)
        if (_italicAngle != 0) flags |= 64; // Italic (bit 7)
        return flags;
    }

    // ----- metrics + cmap helpers -----
    ...
    private void BuildGlyphMap(byte[] d, int cmap)
    {
        ...
        if (best < 0)
        {
            return; // no usable subtable; widths fall back to glyph 0
        }
