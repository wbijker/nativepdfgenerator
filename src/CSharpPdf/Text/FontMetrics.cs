...
/// so measurements match what a conformant reader actually renders. Tables cover
/// printable ASCII (codes 32–126); accented Latin-1 letters reuse their base
/// letter's width (exact for these fonts, where a floating accent does not change
/// the advance).
/// </summary>
public static class FontMetrics
{
    private const int CourierWidth = 600; // Courier is monospaced; every glyph is 600.

    // Index 0 == character code 32 (space) .. index 94 == code 126 (~).
    ...
    /// and the symbolic fonts (Symbol, ZapfDingbats).
    /// </summary>
    public static int GlyphWidth(string baseFont, char c)
    {
        ...
        if (table is null)
        {
            return 600; // Symbol / ZapfDingbats: per-glyph metrics not modelled here.
        }
        if (c is >= ' ' and <= '~')
        {
            return table[c - ' '];
        ...
        return table['0' - ' ']; // neutral fallback (figure width)
    }
    ...
    // Map an accented Latin-1 letter to its base ASCII letter (é -> e, ñ -> n, ...)
    // via Unicode canonical decomposition; non-letters return unchanged.
    private static char BaseLatinLetter(char c)
    {
        switch (c)
        {
            case (char)0xA0: return ' '; // non-breaking space -> space width
            case '×': case '÷': return '+'; // multiply / divide share the plus width
        }
