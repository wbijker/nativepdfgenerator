...
/// colour space — a requirement of standards such as PDF/X and PDF/A. Add a
/// <see cref="DestOutputProfile"/> (ICC stream reference) for full conformance.
/// </summary>
public sealed class OutputIntent
{
    ...
    public PdfDictionary Build()
    {
        ...
        if (Info is not null) d["Info"] = new PdfString(Info);
        if (DestOutputProfile is { } profile) d["DestOutputProfile"] = profile;
        return d;
    }
}
