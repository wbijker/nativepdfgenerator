...
public class MultiColumn : Element
{
    ...
    protected override RenderResult Draw(ContentStream cs, PdfSize available)
    {
        ...
        // Greedy column fill — content flows straight down the current
        // column until an item can't start in the space that's left, then
        // moves to the next column (left → right). No height/weight
        // balancing: the left column is filled first, top to bottom.
        int currentCol = 0;
        double y = 0;
        double maxColY = 0;
        ...
        while (!_iterator.Done)
        {
            ...
            // Move to the next column when even one line of the next item
            // wouldn't fit in the space left. Never wrap while the column is
            // still empty (y == 0) — even an oversize item is drawn there so
