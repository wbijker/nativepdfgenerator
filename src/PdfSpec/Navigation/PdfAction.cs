...
/// <summary>
/// Builds action dictionaries (ISO 32000-1 §12.6, "Actions"). Every action has
/// an <c>/S</c> key declaring its type; remaining keys depend on the type.
/// Named <c>PdfAction</c> to avoid colliding with <see cref="System.Action"/>.
/// </summary>
public static class PdfAction
{
    ...
    /// <summary>GoToR — "remote go-to": open another PDF and jump to a zero-based page index.</summary>
    ...
    /// <summary>GoToE — "embedded go-to": jump into an embedded PDF named in the EmbeddedFiles name tree.</summary>
    public static PdfDictionary GoToEmbedded(string targetName, int pageIndex = 0, string zoom = "Fit")
    {
        ...
        target.SetName("R", "C");
        target.SetString("N", targetName);
        a.Add("T", target);
        return a;
    }

    /// <summary>SubmitForm — send field values to a URL.</summary>
    ...
    public static PdfDictionary ResetForm() => Base("ResetForm");
    ...
    /// <summary>Sound — play a sound stream.</summary>
    ...
    public static PdfDictionary Filespec(string path)
    {
        var d = new PdfDictionary();
        d.SetName("Type", "Filespec");
        d.SetString("F", path);
        d.SetString("UF", path);
        return d;
    }
    ...
    private static PdfDictionary Base(string type, params (string Key, PdfObject Value)[] entries)
    {
        ...
        action.SetName("S", type);
        foreach (var (key, value) in entries) action.Add(key, value);
        return action;
    }
}
